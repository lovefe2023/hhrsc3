      ...
      <div className="px-4 mt-8">
        ...
        <div className="space-y-3">
          ...
          <label 
            className={cn(
              "flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-xl cursor-pointer transition-colors",
              selectedPayment === 'alipay' ? "border-2 border-primary" : "border border-slate-100 dark:border-slate-800 hover:border-primary/50"
            )}
            onClick={() => setSelectedPayment('alipay')}
          >
            <div className="flex items-center gap-4">
              <div className="size-10 rounded-lg bg-[#1677FF]/10 flex items-center justify-center">
                <span className="material-symbols-outlined text-[#1677FF]">payments</span>
              </div>
              <div>
                <p className="text-slate-900 dark:text-slate-100 font-semibold">支付宝</p>
                <p className="text-slate-500 dark:text-slate-400 text-xs">安全便捷的数字钱包</p>
              </div>
            </div>
            ...
            <input type="radio" name="payment" className="hidden" checked={selectedPayment === 'alipay'} readOnly />
          </label>
          ...
          <label 
            className={cn(
              "flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-xl cursor-pointer transition-colors",
              selectedPayment === 'card' ? "border-2 border-primary" : "border border-slate-100 dark:border-slate-800 hover:border-primary/50"
            )}
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-slate-900 dark:text-slate-100 font-semibold">信用卡/借记卡</p>
                <p className="text-slate-500 dark:text-slate-400 text-xs">支持 Visa, Mastercard, 银联</p>
              </div>
            </div>
            ...
            <input type="radio" name="payment" className="hidden" checked={selectedPayment === 'card'} readOnly />
          </label>
        </div>
      </div>
